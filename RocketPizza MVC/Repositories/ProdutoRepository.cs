...
public sealed class ProdutoRepository(AppDbContext context) : IProdutoRepository
{
    public IReadOnlyList<Produto> Listar(bool somenteAtivos=true)
    {
        ...
        while(r.Read())lista.Add(new(){ProdutoId=r.GetInt32(0),CategoriaId=r.GetInt32(1),Categoria=r.GetString(2),Nome=r.GetString(3),Descricao=r.GetString(4),Preco=r.GetDecimal(5),Imagem=r.IsDBNull(6)?null:"img/pizzas/"+Path.GetFileName(r.GetString(6)),Ativo=r.GetBoolean(7)});
        return lista;
    }
