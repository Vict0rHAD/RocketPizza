document.addEventListener("DOMContentLoaded", () => {
    ...
    iniciarHistorico({ isLogged, openAuth, getCurrentUser, saveUserProfile });
    iniciarCardapio();
...
function iniciarHistorico(auth) {
    ...
    const etapas = ["Pedido confirmado", "Preparando pedido", "Seu pedido saiu para entrega", "Seu pedido foi entregue"];
    const etapaDuracao = 60000;
    const seguro = valor => String(valor ?? "").replace(/[&<>'"]/g, caractere => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" })[caractere]);

    function obterEtapa(pedido) {
        if (pedido.canceladoEm) return -1;
        if (!pedido.criadoEm) return 3;
        return Math.min(3, Math.floor((Date.now() - pedido.criadoEm) / etapaDuracao));
    }

    function salvarAlteracao(pedido, alteracoes) {
        Object.assign(pedido, alteracoes);
        auth.saveUserProfile({ ...user, pedidos });
        user = auth.getCurrentUser();
        pedidos = user?.pedidos || pedidos;
    }

    function renderizar() {
        const atual = pedidos.find(pedido => pedido.criadoEm && obterEtapa(pedido) < 3 && !pedido.canceladoEm);
        const anteriores = pedidos.filter(pedido => pedido !== atual);
        lista.innerHTML = `${atual ? renderPedidoAtual(atual) : ""}${anteriores.length ? `
            <div class="historico-separador"><h2>Pedidos anteriores</h2></div>
            ${anteriores.map(renderPedidoAnterior).join("")}` : (!atual ? `<div class="historico-vazio">Você ainda não fez nenhum pedido.</div>` : "")}`;
        ligarEventos();
    }

    function renderPedidoAtual(pedido) {
        const etapa = obterEtapa(pedido);
        const restante = Math.max(0, etapaDuracao - (Date.now() - pedido.criadoEm));
        const segundos = Math.ceil(restante / 1000);
        return `<article class="pedido-atual" data-atual="${pedido.numero}">
            <div class="pedido-atual-topo">
                <div><span class="pedido-em-andamento">Pedido em andamento</span><h2>Pedido #${seguro(pedido.numero)}</h2><p>${seguro(pedido.sabores)} • ${seguro(pedido.total)}</p></div>
                ${etapa === 0 ? `<div class="cancelamento-prazo"><span>Tempo para cancelar</span><strong class="pedido-cronometro">00:${String(segundos).padStart(2, "0")}</strong><button class="btn-cancelar-pedido" type="button">Cancelar pedido</button></div>` : ""}
            </div>
            <div class="rastreamento" aria-label="Andamento do pedido">
                ${etapas.map((nome, indice) => `<div class="rastreamento-etapa ${indice < etapa ? "concluida" : indice === etapa ? "ativa" : ""}"><span class="etapa-icone">${indice < etapa ? "✓" : indice + 1}</span><div><strong>${nome}</strong><small>${indice < etapa ? "Concluído" : indice === etapa ? "Em andamento" : "Aguardando"}</small></div></div>`).join("")}
            </div>
            <button class="btn ver-comprovante" type="button" data-pedido="${pedido.numero}">Ver comprovante</button>
        </article>`;
    }

    function renderPedidoAnterior(pedido) {
        const cancelado = !!pedido.canceladoEm;
        const entregue = !!pedido.criadoEm && obterEtapa(pedido) === 3 && !cancelado;
        return `<article class="historico-item ${cancelado ? "pedido-cancelado" : ""}">
            <div><span class="historico-numero">Pedido #${seguro(pedido.numero)}</span><h2>${seguro(pedido.sabores)}</h2><p>${seguro(pedido.data)} - ${seguro(pedido.total)}</p><span class="historico-status">${cancelado ? "Pedido cancelado" : entregue ? "Pedido entregue" : "Pedido anterior"}</span></div>
            <div class="historico-acoes"><button class="btn ver-comprovante" type="button" data-pedido="${pedido.numero}">Ver comprovante</button>${entregue && !pedido.avaliacao ? `<button class="btn btn-avaliar" type="button" data-pedido="${pedido.numero}">Avaliar pedido</button>` : ""}${pedido.avaliacao ? `<span class="avaliacao-resumo">${"★".repeat(pedido.avaliacao.estrelas)}${"☆".repeat(5 - pedido.avaliacao.estrelas)}</span>` : ""}</div>
        </article>`;
    }

    function abrirModalCancelamento(pedido) {
        const modal = criarModal("modalCancelamentoPedido", `<h2>Por que deseja cancelar?</h2><p>Selecione o principal motivo do cancelamento.</p><form id="formCancelamento"><div class="motivos-cancelamento">${["Fiz o pedido errado", "Tempo de entrega", "Endereço incorreto", "Problema no pagamento", "Desisti da compra"].map((motivo, i) => `<label><input type="radio" name="motivoCancelamento" value="${motivo}" ${i === 0 ? "required" : ""}><span>${motivo}</span></label>`).join("")}</div><label class="modal-label" for="detalhesCancelamento">Mais detalhes (opcional)</label><textarea id="detalhesCancelamento" maxlength="500" placeholder="Conte um pouco mais sobre o motivo"></textarea><div class="modal-actions"><button class="btn btn-secondary fechar-modal" type="button">Voltar</button><button class="btn" type="submit">Confirmar cancelamento</button></div></form>`);
        modal.querySelector(".fechar-modal").onclick = () => modal.classList.remove("active");
        modal.querySelector("form").onsubmit = evento => {
            evento.preventDefault();
            if (Date.now() - pedido.criadoEm >= etapaDuracao) { modal.classList.remove("active"); renderizar(); return; }
            const motivo = new FormData(evento.currentTarget).get("motivoCancelamento");
            const detalhes = modal.querySelector("#detalhesCancelamento").value.trim();
            salvarAlteracao(pedido, { canceladoEm: Date.now(), cancelamento: { motivo, detalhes } });
            modal.classList.remove("active");
            const cartao = pedido.pagamento === "cartao" || pedido.comprovante?.pagamento === "Cartão de crédito";
            mostrarMensagem(cartao ? "Pedido cancelado, o limite utilizado será estornado ao seu cartão de crédito dentro de até 10 minutos" : "Pedido cancelado, o valor estará de volta a sua conta dentro de até 10 minutos");
            renderizar();
        };
    }
    ...
    function abrirModalAvaliacao(pedido) {
        let nota = 0;
        const modal = criarModal("modalAvaliacaoPedido", `<h2>Como foi seu pedido?</h2><p>Sua avaliação ajuda a Rocket Pizza a melhorar.</p><div class="estrelas-avaliacao" role="radiogroup" aria-label="Nota de 1 a 5 estrelas">${[1,2,3,4,5].map(n => `<button type="button" data-nota="${n}" aria-label="${n} estrela${n > 1 ? "s" : ""}">☆</button>`).join("")}</div><label class="modal-label" for="comentarioAvaliacao">Comentário (opcional)</label><textarea id="comentarioAvaliacao" maxlength="500" placeholder="Conte como foi sua experiência"></textarea><div class="modal-actions"><button class="btn btn-secondary fechar-modal" type="button">Agora não</button><button class="btn confirmar-avaliacao" type="button" disabled>Enviar avaliação</button></div>`);
        const estrelas = [...modal.querySelectorAll("[data-nota]")];
        estrelas.forEach(estrela => estrela.onclick = () => { nota = Number(estrela.dataset.nota); estrelas.forEach((item, i) => item.textContent = i < nota ? "★" : "☆"); modal.querySelector(".confirmar-avaliacao").disabled = false; });
        modal.querySelector(".fechar-modal").onclick = () => modal.classList.remove("active");
        modal.querySelector(".confirmar-avaliacao").onclick = () => { salvarAlteracao(pedido, { avaliacao: { estrelas: nota, comentario: modal.querySelector("#comentarioAvaliacao").value.trim(), criadaEm: Date.now() } }); modal.classList.remove("active"); mostrarMensagem("Obrigado! Sua avaliação foi enviada com sucesso."); renderizar(); };
    }

    function criarModal(id, conteudo) {
        let modal = document.getElementById(id);
        if (!modal) { modal = document.createElement("div"); modal.id = id; modal.className = "pedido-modal"; document.body.appendChild(modal); modal.addEventListener("click", evento => { if (evento.target === modal) modal.classList.remove("active"); }); }
        modal.innerHTML = `<div class="pedido-modal-box modal-experiencia"><button class="pedido-modal-close fechar-x" type="button" aria-label="Fechar">&times;</button>${conteudo}</div>`;
        modal.querySelector(".fechar-x").onclick = () => modal.classList.remove("active");
        modal.classList.add("active");
        return modal;
    }

    function mostrarMensagem(texto) {
        const modal = criarModal("modalMensagemPedido", `<div class="mensagem-sucesso-icone">✓</div><h2>Pronto!</h2><p>${texto}</p><button class="btn fechar-modal" type="button">Entendi</button>`);
        modal.querySelector(".fechar-modal").onclick = () => modal.classList.remove("active");
    }

    function ligarEventos() {
        lista.querySelectorAll(".ver-comprovante").forEach(botao => botao.onclick = () => { const pedido = pedidos.find(item => String(item.numero) === String(botao.dataset.pedido)); if (pedido) abrirComprovante(pedido.comprovante); });
        lista.querySelector(".btn-cancelar-pedido")?.addEventListener("click", () => { const pedido = pedidos.find(item => String(item.numero) === String(lista.querySelector("[data-atual]")?.dataset.atual)); if (pedido) abrirModalCancelamento(pedido); });
        lista.querySelectorAll(".btn-avaliar").forEach(botao => botao.onclick = () => { const pedido = pedidos.find(item => String(item.numero) === String(botao.dataset.pedido)); if (pedido) abrirModalAvaliacao(pedido); });
    }

    renderizar();
    const atualizador = setInterval(() => { if (!document.body.contains(lista)) return clearInterval(atualizador); renderizar(); }, 1000);
}
...
function iniciarPedidos(auth) {
    ...
    const tipoMassa = document.getElementById("tipoMassa");
    const recheioMassa = document.getElementById("recheioMassa");
    ...
    if (!saborUm || !saborDois || !tipoPizza || !tamanhoPizza || !tipoMassa || !recheioMassa || !entradaPedido || !quantidadeEntrada || !bebidaPedido || !quantidadeBebida || !adicionarCarrinho) {
        return;
    }
    ...
    function obterResumo() {
        ...
        const massa = tipoMassa.value;
        const recheio = massa === "recheada" ? recheioMassa.value : "";
        return { tamanho, primeiro, segundo, doisSabores, combo, pizza, entrada, bebida, pagamento, desconto, total, massa, recheio };
    }
    ...
    function atualizarResumo() {
        ...
        const massaRecheada = resumo.massa === "recheada";
        document.getElementById("recheioMassaGrupo").hidden = !massaRecheada;
        recheioMassa.required = massaRecheada;
        ...
        document.getElementById("resumoMassa").textContent = resumo.massa === "recheada" ? `Recheada com ${resumo.recheio === "catupiry" ? "Catupiry" : "Cheddar"}` : resumo.massa === "pan" ? "Pan" : "Normal";
    ...
    function montarComprovante(itens, totais) {
        ...
        const pagamento = document.querySelector("input[name='pagamento']:checked").value;
        const saboresTexto = itens.map((item, index) => `${index + 1}. ${item.sabores} (${item.tamanho}, massa ${item.massa})`).join("<br>");
    ...
    function criarItemCarrinho(resumo) {
        ...
        return {
            ...
            massa: resumo.massa === "recheada" ? `Recheada com ${resumo.recheio === "catupiry" ? "Catupiry" : "Cheddar"}` : resumo.massa === "pan" ? "Pan" : "Normal",
            pizza: resumo.pizza,
    ...
    function limparMontagemPizza() {
        ...
        tipoPizza.value = "1";
        tipoMassa.value = "normal";
        recheioMassa.value = "catupiry";
        saborUm.value = "1";
        popularSaboresDois();
        entradaPedido.value = "";
    ...
    [saborUm, saborDois, tipoPizza, tamanhoPizza, tipoMassa, recheioMassa, quantidadeEntrada, quantidadeBebida].forEach(campo => {
